...
```bash
cd servers/postgres-readonly      && npm install && npm test    # 190 SQL-guard (+ SELECT-INTO, comment-merged-keyword, backslash-quote-split & side-effect/admin-function guards incl. adminpack pg_file_*/pg_logdir_ls + signal/promote/wal-replay/log-backend-memory-contexts + backup/rotate-logfile/export-snapshot + replication-slot advance/consume/copy + pg_sync_replication_slots + catalog/index-maintenance pg_import_system_collations/brin/gin/pg_prewarm/autoprewarm/pg_truncate_visibility_map + XID-assigning txid_current/pg_current_xact_id + large-object lowrite/loread) + public-surface + cfg-validation + describe_schema type-resolution + non-string-identifier guards + handler isError-contract + boot-config-failure (spawned; one actionable line, not a stack) + numeric-env grammar (MAX_ROWS/STATEMENT_TIMEOUT_MS reject unit suffixes, scientific notation, separators and precision-losing magnitudes) + DATABASE_URL whitespace (a padded connection string resolves to the same host as the unpadded one; a blank one is refused exactly like an unset one, because `pg` reads ` postgres://…` as a keyword/value string and connects elsewhere as the OS user) tests
cd servers/filesystem-sandbox     && npm install && npm test    # 87 sandbox (+ leaf-symlink write-path guard) + tool (+ non-string content typeof guard) + config (+ MAX_BYTES grammar parity) + max-bytes value-domain parity (shared table with the Python port) + config trim-set parity (shared codepoint table with the Python port) + resolve() parity (shared path table with the Python port) + error-message parity (shared table with the Python port) + public-surface + atomic-write (+ NAME_MAX-basename temp-name) + boot-config-failure (spawned; one actionable line, not a stack) tests
cd servers/github-gists           && npm install && npm test    # 98 config + client (redaction + rate-limit diag + cfg validation + filename-trim + non-string gist_id/filename/description typeof guard + single-read-Response error body + capped JSON error message) + timeoutMs setTimeout-clamp upper bound (env grammar + programmatic) + error-message-format + boot-config-failure (spawned; one actionable line, not a stack) + numeric-env grammar (TIMEOUT_MS refuses hex/scientific notation, which bare `Number()` accepted) + diagnostic-header grammar (the same gate on GitHub's rate-limit headers, mapping to `null` instead of throwing; `0x10` no longer reads as a rate-limit of 0) + tool + public-surface tests
...
cd servers/filesystem-sandbox-py  && pip install -e '.[dev]' && pytest  # 132 sandbox + tool (+ non-string content typeof guard) + config (+ MAX_BYTES grammar parity) + max-bytes value-domain parity (shared table with the TS port) + config trim-set parity (shared codepoint table with the TS port) + resolve() parity (shared path table with the TS port) + error-message parity (shared table with the TS port, + lone-surrogate rows and a transport-encodability property) + public-surface + atomic-write (+ NAME_MAX-basename temp-name, + filesystem-byte name-encoding with a shared surrogateescape-range table) + isError-parity (wire-level, SDK-major-stable) + ruff-config-scope tests
```
