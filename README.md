...
```bash
cd servers/postgres-readonly      && npm install && npm test    # 110 SQL-guard (+ SELECT-INTO, comment-merged-keyword, backslash-quote-split & side-effect-function guards) + public-surface + cfg-validation + describe_schema type-resolution + handler isError-contract tests
