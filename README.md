...
cd servers/filesystem-sandbox     && npm install && npm test    # 87 sandbox (+ leaf-symlink write-path guard) + tool (+ non-string content typeof guard) + config (+ MAX_BYTES grammar parity) + max-bytes value-domain parity (shared table with the Python port) + public-surface + atomic-write (+ NAME_MAX-basename temp-name) tests
...
cd servers/filesystem-sandbox-py  && pip install -e '.[dev]' && pytest  # 122 sandbox + tool (+ non-string content typeof guard) + config (+ MAX_BYTES grammar parity) + max-bytes value-domain parity (shared table with the TS port) + public-surface + atomic-write (+ NAME_MAX-basename temp-name) + isError-parity (wire-level, SDK-major-stable) + ruff-config-scope tests
```
