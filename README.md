...
cd servers/filesystem-sandbox     && npm install && npm test    # 63 sandbox (+ leaf-symlink write-path guard) + tool (+ non-string content typeof guard) + config (+ MAX_BYTES grammar parity) + max-bytes value-domain parity (shared table with the Python port) + public-surface + atomic-write (+ NAME_MAX-basename temp-name) tests
