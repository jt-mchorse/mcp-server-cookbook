...
```bash
cd servers/postgres-readonly      && npm install && npm test    # 83 SQL-guard (+ SELECT-INTO, comment-merged-keyword & backslash-quote-split guards) + public-surface + cfg-validation + describe_schema type-resolution tests
...
cd servers/internal-tools-bridge  && npm install && npm test    # 33 bridge + tool + public-surface tests (no shell, env scrub, output cap, wall-clock-bound timeout, validateConfig)
cd servers/filesystem-sandbox-py  && pip install -e '.[dev]' && pytest  # 71 sandbox + tool + config + public-surface + atomic-write tests
```
